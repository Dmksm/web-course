...
class SurveyView
{
    ...
    {
        $surveyArray = ['', '', '', ''];
        $defoltTitle = "Don't found email!";
        if ($survey && (file_exists("./data/" . $survey->getParameterEmail() . ".txt")))
        {
            $surveyArray = ['First name: ' . SurveyPrinter::printFirstName($survey), 'Last name: ' . SurveyPrinter::printLastName($survey), 'Email: ' . SurveyPrinter::printEmail($survey), 'Age: ' . SurveyPrinter::printAge($survey)];
            $defoltTitle = $title;
        }
        return ['operation_with_survey' => $defoltTitle,
